...
class Decoder(implicit val p: NutCoreConfig) extends NutCoreModule with HasInstrType {
  ...
  val imm = LookupTree(instrType, List(
    ...
    InstrS  -> SignExt(Cat(instr(31, 25), instr(11, 7)), VLEN),
    InstrSA -> SignExt(Cat(instr(31, 25), instr(11, 7)), VLEN),
    //fft扩展指令对于立即数的扩展
    //每次的立即数只能写16bits 对于128bits的regvec需要8次，即需要8位来进行控制
    InstrFFT-> ZeroExt(Cat(instr(31, 16)), VLEN),
    InstrB  -> SignExt(Cat(instr(31), instr(7), instr(30, 25), instr(11, 8), 0.U(1.W)), VLEN),
    InstrU  -> SignExt(Cat(instr(31, 12), 0.U(12.W)), VLEN),//fixed
    InstrJ  -> SignExt(Cat(instr(31), instr(19, 12), instr(20), instr(30, 21), 0.U(1.W)), VLEN)
  ))
