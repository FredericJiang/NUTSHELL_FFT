...
object RV64IInstr extends HasInstrType {
  ...
  def LDVEC0  = BitPat("b???????_?????_?????_110_?????_0100011")
  def LDVEC1  = BitPat("b???????_?????_?????_111_?????_0100011")
  def SDVEC0  = BitPat("b???????_?????_?????_100_?????_0100011")
  def SDVEC1  = BitPat("b???????_?????_?????_101_?????_0100011")
  ...
  val table = Array(
    ...
    LDVEC0      -> List(InstrI, FuType.lsu, LSUOpType.ldvec0),
    LDVEC1      -> List(InstrI, FuType.lsu, LSUOpType.ldvec1),
    SDVEC0      -> List(InstrS, FuType.lsu, LSUOpType.sdvec0),
    SDVEC1      -> List(InstrS, FuType.lsu, LSUOpType.sdvec1)
  )
}
