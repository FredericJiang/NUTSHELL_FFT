...
//load store instruction for rvec
  def LDVEC0        = BitPat("b???????_?????_?????_000_?????_1111011")
  def LDVEC1        = BitPat("b???????_?????_?????_001_?????_1111011")
  def SDVEC0        = BitPat("b???????_?????_?????_010_?????_1111011")
  def SDVEC1        = BitPat("b???????_?????_?????_011_?????_1111011")
  ...
  val table = Array(
    ...
    LDVEC0         -> List(InstrI, FuType.lsu, LSUOpType.ld),
    LDVEC1         -> List(InstrI, FuType.lsu, LSUOpType.ld),
    SDVEC0         -> List(InstrI, FuType.lsu, LSUOpType.sd),
    SDVEC1         -> List(InstrI, FuType.lsu, LSUOpType.sd),
