...
sealed class Stage1IO(implicit val cacheConfig: CacheConfig) extends CacheBundle {
  val req = new SimpleBusReqBundle(userBits = userBits, idBits = idBits)  // 包括指令的全部读写信息：读or写，size，addr，id
}

// meta read
sealed class CacheStage1(implicit val cacheConfig: CacheConfig) extends CacheModule {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new SimpleBusReqBundle(userBits = userBits, idBits = idBits)))// 输入指令的全部读写信息：读or写，size，addr，id
  ...
  //输入指令的全部信息，再输出，为何要有cache stage 1？
  //只通过metaReadBus和dataReadBus传递index信息
...
sealed class Stage2IO(implicit val cacheConfig: CacheConfig) extends CacheBundle {
  ...
  val forwardData = Output(CacheDataArrayWriteBus().req.bits)//写数据的 data, waymask, index信息
}
...
sealed class CacheStage2(implicit val cacheConfig: CacheConfig) extends CacheModule {
  val io = IO(new Bundle {
    ...
    val metaReadResp = Flipped(Vec(Ways, new MetaBundle))//input tag、valid、dirty
    val dataReadResp = Flipped(Vec(Ways, new DataBundle))//input data
...
// RegEnale == RegNext（updateData，enable） 有控制信号的reg，当enable为真时，data在下一拍输出 
  ...
  }//zipWithIndex函数遍历forwardWaymask数据,返回一个二元数组

  val hitVec = VecInit(metaWay.map(m => m.valid && (m.tag === addr.tag) && io.in.valid)).asUInt
  val victimWaymask = if (Ways > 1) (1.U << LFSR64()(log2Up(Ways)-1,0)) else "b1".U //通过随机数产生victim line 用作cache替换
...
sealed class CacheStage3(implicit val cacheConfig: CacheConfig) extends CacheModule {
  ...
  val useForwardData = io.in.bits.isForwardData && io.in.bits.waymask === io.in.bits.forwardData.waymask.getOrElse("b1".U)//getorelse 如果waymask无值则默认赋1
...
//通过datareadbus，读取cache中没有的数据
  ...
  // critical word first read 用于refill dirty的victim line
...
//每个通道的优先级不一样，hitwrite的优先级高于fillwrite
...
class Cache(implicit val cacheConfig: CacheConfig) extends CacheModule {
  ...
  */
  ...
  io.in.resp <> s3.io.out//最终输出访存指令所需的数据
  s3.io.flush := io.flush(1)
  io.out.mem <> s3.io.mem//向mem的输出
  io.mmio <> s3.io.mmio
  ...
  s2.io.metaReadResp := s1.io.metaReadBus.resp.data//stage 1 根据地址输出的数据 输入进入stage 2，进行tag dirty比较？
