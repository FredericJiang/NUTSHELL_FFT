...
class SRAMReadBus[T <: Data](private val gen: T, val set: Int, val way: Int = 1) extends Bundle {
  val req = Decoupled(new SRAMBundleA(set))//接收地址的index
  val resp = Flipped(new SRAMBundleR(gen, way))//根据tag、valid、dirty 构建 meta array 或者根据data内容构建 data array
...
class SRAMTemplateWithArbiter[T <: Data](nRead: Int, gen: T, set: Int, way: Int = 1,
  ...
  val io = IO(new Bundle {
    val r = Flipped(Vec(nRead, new SRAMReadBus(gen, set, way)))//有几路读接口
    val w = Flipped(new SRAMWriteBus(gen, set, way)) //写接口
  })

  val ram = Module(new SRAMTemplate(gen, set, way, shouldReset, holdRead = false, singlePort = true))
  //存储模块，有set way等参数可以配置
  ram.io.w <> io.w

//Arbiter为chisel3自带的类，本例中输入的传输数据类型为io.r(0).req.bits，待仲裁的个数为nread给出
//Arbiter会根据优先级进行输出，每个通道的优先级不同，把高优先级的信号接入0通道，依次类推
